...
def vehicule_disponible(cur,inmat,debut,fin):
        return
...
def reserver_place_cible(cur, debut, fin, idvehicule, client, idParking, idPlace):
    if est_Reserve(idPlace, idParking, debut, fin):
        print("reservation impossible, la place est déjà réservée")
    else:
        sql = "INSERT into Reservation values (Default,'%s','%s','%s','%s','%s','%s')", (
            debut, fin, idvehicule, client, idParking, idPlace)
        cur.execute(sql)
